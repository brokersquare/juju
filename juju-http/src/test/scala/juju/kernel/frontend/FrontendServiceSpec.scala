...
class FrontendServiceSpec extends FlatSpec with Matchers with ScalatestRouteTest with PingService with DefaultJsonProtocol  {
...
*/
  ...
  */
  ...
  private def commandProxyFactory: CommandProxyFactory = new FakeCommandProxyFactory(system)
  private def commandGateway() = commandProxyFactory.actor

  private def commandGatewayRoute[C <: Command : FromRequestUnmarshaller] = {
...
//  import CommandsJsonProtocol._
  ...
  override val apiRoute: Route = {
    pathPrefix("api") {
      post {
        path("fake") {
          commandGatewayRoute[FakeSimpleCommand]
        } ~
          path("fake") {
            commandGatewayRoute[FakeSimpleWithNumberParameterCommand]
          }
      } ~ pingRoute
    }
  }
